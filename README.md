...
Unlike Google Docs, edits happen on the actual markdown — so the file in your repo stays the source of truth and the round-trip back to GitHub 
...
- **Review requests + standing reviewers** — one click summons a reviewer (a person who's touched the doc, or one of your agent tokens). Fulfillment is implicit: the request completes the moment they set a review state. Check "Also review every future revision" and they become a *standing reviewer* — every new revision automatically re-summons them (CI for prose). Pending requests show as "awaiting …" chips so nobody double-asks.
- **Doc checks + named policies** — CI-style lint rules for prose: required sections (picked from the doc's own headings), banned words/phrases, exact term spelling ("Beamable", never "beamable"), heading-depth caps, and raw regex for power users — every rule live-previews against the doc while you compose it. Save a rule set as a **named policy** ("PRD Standard") and apply it anywhere from a dropdown; docs *link* to the policy, so improving it updates every linked doc instantly (editing while linked makes the blast radius explicit: save to the policy, or fork for this doc only). On an index you created, map filename patterns to policies — `_PRD → PRD Standard` — with per-file exceptions; matching docs link on first open, forever. The doc's Checks button carries the verdict: green ✓ or red ✗ with the failing count.
- **Auto-review tokens** — flag a token as *auto-review* and the server itself fulfills review requests targeting it: Claude reads the doc and open discussion, leaves anchored suggestions with rationales, and sets an honest review state — within a minute of any revision, billed to your own Anthropic key. Combined with standing reviewers this is CI for prose: save a revision, get a review, click Apply.
- **Comments survive rewrites** — when the upstream text changes, a conservative fuzzy matcher re-anchors comments through punctuation churn, word swaps, and reflowed sentences instead of orphaning them. Recovered anchors show a small "≈" hint with the original selection preserved.
...
- **Review requests + standing reviewers**: a request targets a human (bell notification) or an agent token (polls via the `list_review_requests` MCP tool). Standing subscriptions anchor to the chain root; a fan-out hook in every revision-creation path mints fresh requests per subscriber, skipping the revision's author. See [backend/internal/api/review_requests.go](backend/internal/api/review_requests.go) + [review_subscriptions.go](backend/internal/api/review_subscriptions.go).
- **Fuzzy anchor recovery**: rune-trigram Dice scoring over sliding windows, accepting only with high absolute similarity AND a clear margin over the best non-overlapping candidate — a wrong re-anchor is worse than an orphan. See [backend/internal/api/fuzzyanchor.go](backend/internal/api/fuzzyanchor.go).
- **Admin console**: `/admin`, gated by the `MARKUPMARKDOWN_ADMIN_LOGINS` env allowlist (cookie-session only; 404 to non-admins). Usage counts, public/private split, 30-day activity, and a public-docs-only feed enforced in the query. See [backend/internal/api/admin.go](backend/internal/api/admin.go).
